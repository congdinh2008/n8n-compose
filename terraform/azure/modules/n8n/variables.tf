...
variable "use_spot_instance" {
  description = "Whether to use Azure Spot VM (can reduce costs but may be evicted)"
  type        = bool
  default     = false
}

variable "spot_max_price" {
  description = "The maximum price you're willing to pay for the spot VM (-1 means pay the on-demand price)"
  type        = number
  default     = -1
}

variable "spot_eviction_policy" {
  description = "The eviction policy for the Spot VM (Deallocate or Delete)"
  type        = string
  default     = "Deallocate"
  validation {
    condition     = contains(["Deallocate", "Delete"], var.spot_eviction_policy)
    error_message = "Spot eviction policy must be either 'Deallocate' or 'Delete'."
  }
}
